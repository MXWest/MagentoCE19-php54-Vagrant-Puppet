class magento($db_username, $db_password, $version, $admin_username, $admin_password, $use_rewrites) {
  ...
  exec { "setting-permissions":
    ...
    command => "/bin/chmod 550 mage; /bin/chmod o+w var var/.htaccess app/etc; /bin/chmod -R o+w media; find . -exec chmod g+w {} \\;; find . -type d -exec chmod g+w {} \\;",
  ...
  exec { "register-magento-channel":
    ...
    command => "/bin/bash ${apache2::document_root}/magento/mage mage-setup",
